...
/** This class implements a munit test suite for the methods in object `Lists`
  * that need to be implemented as part of this assignment. A test suite is
  * simply a collection of individual tests for some specific component of a
  * program.
  *
...
class ListsSuite extends munit.FunSuite:
  ...
  /** Tests are written using the `test("description") { ... }` syntax The most
    * common way to implement a test body is using the method `assert` which
    * tests that its argument evaluates to `true`. So one of the simplest
  ...
  /** One problem with the previous (failing) test is that munit will only tell
    * you that a test failed, but it will not tell you what was the reason for
    * the failure. The output looks like this:
    *
    * {{{
    ...
    * }}}
    *
    * This situation can be improved by using a assertEquals (this is only
    * possible in munit). So if you run the next test, munit will show the
    * following output:
    *
    * {{{
    ...
    * }}}
    *
    * We recommend to always use the assertEquals equality operator when writing
    * tests.
    */
  ...
  /** Exceptional behavior of a methods can be tested using a try/catch and a
    * failed assertion.
    *
  ...
  /** Now we finally write some tests for the list functions that have to be
    * implemented for this assignment. We fist import all members of the `List`
    * object.
    */
  import Lists.*
  ...
  test("sum list with zeros") {
    assert(sum(List(1, 4, 0)) == 5)
  }

  test("sum list with negative numbers") {
    assert(sum(List(1, -4, -3, 6)) == 0)
  }
