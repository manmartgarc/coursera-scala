...
object TimeUsage extends TimeUsageInterface:
  ...
  def classifiedColumns(
    ...
    val (
      ...
      columnNames.foldLeft(
        (List.empty[Column], List.empty[Column], List.empty[Column])
      ) {
        case (
              (
                primaryNeedsColumns: List[Column],
                workingActivitiesColumns: List[Column],
                otherActivitiesColumns: List[Column]
              ),
              columnName: String
            ) =>
          if primaryNeeds.exists(columnName.startsWith) then
            (
              col(columnName) :: primaryNeedsColumns,
              workingActivitiesColumns,
              otherActivitiesColumns
            )
          else if workingActivities.exists(columnName.startsWith) then
            (
              primaryNeedsColumns,
              col(columnName) :: workingActivitiesColumns,
              otherActivitiesColumns
            )
          else if otherActivities.exists(columnName.startsWith) then
            (
              primaryNeedsColumns,
              workingActivitiesColumns,
              col(columnName) :: otherActivitiesColumns
            )
          else
            (
              primaryNeedsColumns,
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when($"telfs" < 3, "working")
      .otherwise("not working")
      .as("working")
    val sexProjection: Column =
      when($"tesex" === 1, "male").otherwise("female").as("sex")
    val ageProjection: Column = when($"teage" <= 22, "young")
    ...
    val primaryNeedsProjection: Column = df
      .select(
        primaryNeedsColumns: _*
      )
      .columns
      .map(col)
      .reduce(_ + _)
      .divide(60)
      .as("primaryNeeds")
    val workProjection: Column = df
      .select(workColumns: _*)
      .columns
      .map(col)
      .reduce(_ + _)
      .divide(60)
      .as("work")
    val otherProjection: Column = df
      .select(otherColumns: _*)
      .columns
      .map(col)
      .reduce(_ + _)
      .divide(60)
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame =
    summed
      ...
      .agg(
        round(mean($"primaryNeeds"), 1).as("primaryNeeds"),
        round(mean($"work"), 1).as("work"),
        round(mean($"other"), 1).as("other")
      )
  ...
  def timeUsageGroupedTyped(
    ...
    def myRound(d: Double): Double = (d * 10).round / 10.0
    summed
      ...
      .map { case ((working, sex, age), pn, w, o) =>
        TimeUsageRow(working, sex, age, myRound(pn), myRound(w), myRound(o))
      }
      .orderBy("working", "sex", "age")
