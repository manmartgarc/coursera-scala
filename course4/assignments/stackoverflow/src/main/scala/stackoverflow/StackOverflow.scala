...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def scoredPostings(
    ...
    grouped.mapValues(
      vs =>
        val (q, as) = vs.head
        (q, answerHighScore(vs.map(_._2).toArray))
    ).values
  ...
  def vectorPostings(
    ...
    scored.flatMap { case (question, highScore) =>
      firstLangInTag(question.tags, langs) match
        case None      => None
        case Some(idx) => Some(idx * langSpread, highScore)
    }
  ...
  @tailrec final def kmeans(
    ...
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = vectors
  ...
  def clusterResults(
    ...
    def getMedian(xs: Seq[Int]): Int =
      val (lower, upper) = xs.sortWith(_ < _).splitAt(xs.size / 2)
      if xs.size % 2 == 0 then (lower.last + upper.head) / 2 else upper.head
    ...
    val median = closestGrouped.mapValues { vs =>
      val groupByLang =
        vs.map(_._1 / langSpread).groupBy(identity).mapValues(_.size)
      val mostCommonLang = groupByLang.maxBy(_._2)._1
      val langLabel: String =
        langs(mostCommonLang) // most common language in the cluster
      val langPercent: Double =
        groupByLang(
          mostCommonLang
        ) * 100.0 / vs.size // percent of the questions in the most common language
      val clusterSize: Int = vs.size
      val clusterScores = vs.map(_._2).toSeq
