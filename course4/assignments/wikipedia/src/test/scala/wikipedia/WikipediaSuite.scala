...
class WikipediaSuite extends munit.FunSuite:
  ...
  /** Creates a truncated string representation of a list, adding ", ...)" if
    * there are too many elements to show
    * @param l
    *   The list to preview
    * @param n
    *   The number of elements to cut it at
    * @return
    *   A preview of the list, containing at most n elements.
    */
  def previewList[A](l: List[A], n: Int = 10): String =
    if l.length <= n then l.toString
    else l.take(n).toString.dropRight(1) + ", ...)"

  /** Asserts that all the elements in a given list and an expected list are the
    * same, regardless of order. For a prettier output, given and expected
  ...
  def assertSameElements[A](actual: List[A], expected: List[A]): Unit =
    ...
    assert(
      noUnexpectedElements,
      s"""|$noMatchString
          |The given collection contains some unexpected elements: ${previewList(
           unexpected.toList,
           5
         )}""".stripMargin
    )
    ...
    assert(
      noMissingElements,
      s"""|$noMatchString
          |The given collection is missing some expected elements: ${previewList(
           missing.toList,
           5
         )}""".stripMargin
    )
  ...
  def assertEquivalentAndOrdered(
    ...
    // (2)
    assert(
      !(actual zip actual.tail).exists({ case ((_, occ1), (_, occ2)) =>
        occ1 < occ2
      }),
  ...
  test(
    "'rankLangsUsingIndex' should work for a simple RDD with three elements"
  ) {
    assert(
      initializeWikipediaRanking(),
      " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?"
  ...
  test(
    "'rankLangsReduceByKey' should work for a simple RDD with five elements"
  ) {
    assert(
      initializeWikipediaRanking(),
      " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?"
