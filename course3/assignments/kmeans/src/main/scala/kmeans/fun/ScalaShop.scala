...
object ScalaShop:
  ...
  class ScalaShopFrame extends JFrame("ScalaShop\u2122"):
    ...
    val etaCountSpinner = JSpinner(
      SpinnerNumberModel(0.001, 0.00001, 0.01, 0.00001)
    )
      ...
      def actionPerformed(e: ActionEvent): Unit = {
        ...
        val time = measure {
          status = canvas.applyIndexedColors(
            getColorCount,
            getInitialSelectionStrategy,
            getConvergenceStragegy
          )
        }
    ...
    def getInitialSelectionStrategy: InitialSelectionStrategy =
      if randomSamplingButton.isSelected() then RandomSampling
      else if uniformSamplingButton.isSelected() then UniformSampling
      else UniformChoice
    ...
    def getConvergenceStragegy: ConvergenceStrategy =
      if stepConvergenceButton.isSelected() then
        ...
        ConvergedAfterMeansAreStill(
          etaCountSpinner.getValue.asInstanceOf[Double]
        )
      else ConvergedWhenSNRAbove(snrCountSpinner.getValue.asInstanceOf[Int])
