...
class KMeans extends KMeansInterface:
  ...
  def classify(
    ...
    points.par.groupBy(findClosest(_, means))
  ...
  final def kMeans(
    ...
    if converged(eta, means, update(classify(points, means), means)) then means
    else kMeans(points, update(classify(points, means), means), eta)
