...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase
      ...
      .map((char: Char, str: String) => (char, str.length))
      .toList
      .sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    val yMap = y.toMap
    x.map((char, count) => (char, count - yMap.getOrElse(char, 0)))
    ...
    def sentenceAnagramsHelper(occurrences: Occurrences): List[Sentence] =
      if occurrences.isEmpty then List(List())
        ...
        for
          ...
          sentence <- sentenceAnagramsHelper(subtract(occurrences, combination))
        yield word :: sentence
