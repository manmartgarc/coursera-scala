...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def union(that: TweetSet): TweetSet =
    // Avoid exponential time complexity: https://tinyurl.com/mphepfhk
  ...
  def mostRetweeted: Tweet =
    if left.isEmpty && right.isEmpty then elem
      ...
      if r.retweets > elem.retweets then r else elem
    else if right.isEmpty then
      val l = left.mostRetweeted
      if l.retweets > elem.retweets then l else elem
    else
      val l = left.mostRetweeted
      val r = right.mostRetweeted
      if l.retweets > elem.retweets then
        if l.retweets > r.retweets then l else r
      else if elem.retweets > r.retweets then elem
      else r
