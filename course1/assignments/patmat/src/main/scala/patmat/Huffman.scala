...
/** A huffman code is represented by a binary tree.
  ...
  * the tree can encode. The weight of a `Leaf` is the frequency of appearance
  * of the character.
  *
  * The branches of the huffman tree, the `Fork` nodes, represent a set
  * containing all the characters present in the leaves below it. The weight of
  * a `Fork` node is the sum of the weights of these leaves.
  */
...
trait Huffman extends HuffmanInterface:
  ...
  /** In this assignment, we are working with lists of characters. This function
    * allows you to easily create a character list from a given string.
    */
  ...
  /** This function computes for each unique character in the list `chars` the
    ...
    *
    * pair match { case (theChar, theInt) => println("character is: "+ theChar)
    * println("integer is : "+ theInt) }
    */
  def times(chars: List[Char]): List[(Char, Int)] = chars match
    case Nil     => Nil
    case x :: xs => (x, xs.count(_ == x) + 1) :: times(xs.filter(_ != x))

  /** Returns a list of `Leaf` nodes for a given frequency table `freqs`.
    *
    * The returned list should be ordered by ascending weights (i.e. the head of
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    ...
    freqs.sortBy(_._2) match
      case head :: next => Leaf(head._1, head._2) :: makeOrderedLeafList(next)
      case Nil          => Nil

  /** Checks whether the list `trees` contains only one single code tree.
    */
  def singleton(trees: List[CodeTree]): Boolean = trees match
    case head :: next => next.isEmpty
    case Nil          => false
  ...
  /** This function creates a code tree which is optimal to encode the text
    ...
    * them.
    */
  def createCodeTree(chars: List[Char]): CodeTree = until(
    singleton,
    combine
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    def find(tree: CodeTree)(ch: Char): List[Bit] = tree match
      case Leaf(_, _)                                => List()
      case Fork(l, r, _, _) if chars(l).contains(ch) => 0 :: find(l)(ch)
      case Fork(l, r, _, _)                          => 1 :: find(r)(ch)
